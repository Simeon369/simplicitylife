...
export default function EditPost() {
    ...
    const fetchPost = async () => {
      try {
        ...
        if (res.ok && json.success && json.data) {
          const post = json.data as BlogPost;
          setTitle(post.title);
          let bodyContent = post.body;
          if (typeof post.body === "string") {
            const { isTiptapJSON, migratePlainTextToBlocks } = await import(
              "@/components/editor/utils/migratePlainText"
            );
            bodyContent = isTiptapJSON(post.body)
              ? JSON.parse(post.body)
              : migratePlainTextToBlocks(post.body);
          }
          setBody(bodyContent);
          setExistingImageUrl(post.headerImage);
          setHeaderImageFile(null, post.headerImage);
  ...
  const handleSubmit = async (status: "draft" | "published") => {
    ...
    try {
      ...
      let headerImageUrl = existingImageUrl;
      if (headerImageFile) {
        const formDataUpload = new FormData();
        formDataUpload.append("file", headerImageFile);
      ...
      {successMessage && (
        <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-50 max-w-md w-full px-4">
          <motion.div
            ...
            className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg shadow-lg flex items-center justify-between"
          >
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        >
          ...
          <div className="min-h-[400px]">
            <SimpleEditor placeholder="Edit your story..." />
          </div>
        </motion.div>
        ...
        >
          ...
          <ul className="text-sm text-gray-600 space-y-2">
            ...
            <li>• Use "Save Draft" to keep changes without publishing</li>
            <li>• Use "Update & Publish" to make changes live immediately</li>
            <li>• Removing an image will delete it from storage</li>
            <li>• You can preview your post from the admin dashboard</li>
          </ul>
        </motion.div>
      </div>
    </div>
